    ...
    public partial class WebHelper : IWebHelper
    {
        ...
        public virtual string GetCurrentIpAddress()
        {
            ...
            try
            {
                ...
                {
                    var forwardedHttpHeaderKey = OSSConfig.XForwardedForHeader;
        ...
        public virtual string GetThisPageUrl(bool includeQueryString, bool? useSsl = null, bool lowercaseUrl = false)
        {
            ...
            //get store location
            var UrlLocation = GetCurrentLocation(useSsl ?? IsCurrentConnectionSecured());

            //add local path to the URL
            var pageUrl = $"{UrlLocation.TrimEnd('/')}{_httpContextAccessor.HttpContext.Request.Path}";
        ...
        public virtual bool IsCurrentConnectionSecured()
        {
            ...
            //    return _httpContextAccessor.HttpContext.Request.Headers[OSSDefaults.HttpClusterHttpsHeader].ToString().Equals("on", StringComparison.OrdinalIgnoreCase);

            ////use HTTP_X_FORWARDED_PROTO?
            //if (_hostingConfig.UseHttpXForwardedProto)
            //    return _httpContextAccessor.HttpContext.Request.Headers[OSSDefaults.HttpXForwardedProtoHeader].ToString().Equals("https", StringComparison.OrdinalIgnoreCase);
        ...
        public virtual string GetCurrentLocation(bool? useSsl = null)
        {
            ...
            if (!string.IsNullOrEmpty(storeHost))
            {
                ...
                UrlLocation = IsRequestAvailable() ? $"{storeHost.TrimEnd('/')}{_httpContextAccessor.HttpContext.Request.PathBase}" : storeHost;
            }

            //ensure that URL is ended with slash
            UrlLocation = $"{UrlLocation.TrimEnd('/')}/";
        ...
        public virtual string ModifyQueryString(string url, string key, params string[] values)
        {
            ...
            if (isLocalUrl)
            {
                ...
                uriStr = $"{GetCurrentLocation().TrimEnd('/')}{(url.StartsWith(pathBase) ? url.Replace(pathBase, "") : url)}";
            }
        ...
        public virtual string RemoveQueryString(string url, string key, string value = null)
        {
            ...
            var urlHelper = _urlHelperFactory.GetUrlHelper(_actionContextAccessor.ActionContext);
            var isLocalUrl = urlHelper.IsLocalUrl(url);
            var uri = new Uri(isLocalUrl ? $"{GetCurrentLocation().TrimEnd('/')}{url}" : url, UriKind.Absolute);
        ...
        public virtual bool IsPostBeingDone
        {
            get
            {
                if (_httpContextAccessor.HttpContext.Items[OSSConfig.IsPostBeingDoneRequestItem] == null)
                    return false;

                return Convert.ToBoolean(_httpContextAccessor.HttpContext.Items[OSSConfig.IsPostBeingDoneRequestItem]);
            }

            set => _httpContextAccessor.HttpContext.Items[OSSConfig.IsPostBeingDoneRequestItem] = value;
        }
        ...
        public virtual object ResolveUnregistered(Type type)
        {
            ...
            foreach (var constructor in type.GetConstructors())
            {
                try
                {
                    ...
                    {
                        var service = _httpContextAccessor.HttpContext.RequestServices.GetService(parameter.ParameterType);
